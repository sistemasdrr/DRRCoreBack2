...
namespace DRRCore.Application.Main.CoreApplication
{
    public class ComboboxApplication : IComboboxApplication
    {
        ...
        public async Task<Response<List<GetComboNameValueResponseDto>>> GetReporters()
        {
            ...
            {
                using var context = new SqlCoreContext();
                var list = await context.Personals.Where(x => x.Enable == true && x.Type == "RP").Include(x => x.IdEmployeeNavigation).OrderBy(x => x.Code).ToListAsync();
