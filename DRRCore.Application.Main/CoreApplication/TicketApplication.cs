...
namespace DRRCore.Application.Main.CoreApplication
{
    public class TicketApplication : ITicketApplication
    {
            ...
            try
            {
                ...
                if (request.Id == 0)
                {
                    ...
                    if (await _ticketDomain.AddAsync(newTicket))
                    {
                        ...
                        var ticketToCoupon = await _ticketDomain.GetByIdAsync(newTicket.Id);
                        if (ticketToCoupon.IdSubscriberNavigation.FacturationType == "CC" && ticketToCoupon.IsComplement == false)
                        {
                            var couponBilling = await context.CouponBillingSubscribers.Where(x => x.IdSubscriber == ticketToCoupon.IdSubscriber).FirstOrDefaultAsync();
                            ...
                            switch (ticketToCoupon.ProcedureType)
                            {
                                case "T2":
                            ...
                            couponBillingHistory.IdTicket = ticketToCoupon.Id;
            ...
            try
            {
                ...
                if (list != null)
                {
                    ...
                    foreach (var item in response.Data)
                    {
                        if (item.About == "E")
                        {
                            var reporter = await context.TicketHistories.Include(x => x.IdTicketNavigation)
                                  .Include(x => x.IdTicketNavigation.IdSubscriberNavigation).Where(x => x.IdTicketNavigation.IdCompany == item.IdCompany && (x.AsignationType == "AG" ||(x.AsignedTo.Substring(0,1)=="R" && x.AsignationType == "RP"))
                              && (x.IdTicketNavigation.IdStatusTicket == (int)TicketStatusEnum.Despachado || x.IdTicketNavigation.IdStatusTicket == (int)TicketStatusEnum.Despachado_con_Observacion) && x.IdTicketNavigation.Id != item.IdTicket).OrderByDescending(x => x.IdTicketNavigation.DispatchtDate).ToListAsync();

                           var lastReport= string.Join("/", reporter.Select(x=>x.AsignedTo).Distinct().ToList());
                            if (reporter.Count > 0)
                            {
                                item.LastReporterAgent = string.Format("{0}-{1}({2})", reporter.First().IdTicketNavigation.IdSubscriberNavigation.Code, lastReport, reporter.First().IdTicketNavigation.Quality);
                            }
                            else
                            {
                                item.LastReporterAgent = string.Empty;
                            }

                            

                        }
                        else if (item.About == "P") {
                            var reporter = await context.TicketHistories.Include(x => x.IdTicketNavigation)
                                .Include(x => x.IdTicketNavigation.IdSubscriberNavigation).Where(x => x.IdTicketNavigation.IdPerson == item.IdPerson && (x.AsignationType == "AG" || (x.AsignedTo.Substring(0, 1) == "R" && x.AsignationType == "RP"))
                            && (x.IdTicketNavigation.IdStatusTicket == (int)TicketStatusEnum.Despachado || x.IdTicketNavigation.IdStatusTicket == (int)TicketStatusEnum.Despachado_con_Observacion) && x.IdTicketNavigation.Id != item.IdTicket).OrderByDescending(x => x.IdTicketNavigation.DispatchtDate).ToListAsync();
                                ...
                                if (ticket != null)
                                {
                                    ...
                                    if (item.Type == "PA")
                                    {
                                       string nameAssignedTo = "PRE_ASSIGN_" + item.AssignedToCode.Trim();
                                        ...
                                        history.HasBalance = item.HasBalance==null?false:item.HasBalance.Value;
                                        ticket.UpdateDate = DateTime.Now;
                                        ticket.IdStatusTicket = (int)TicketStatusEnum.Pre_Asignacion;
        ...
        public async Task<Response<bool>> DispatchTicket(int idTicket, int idUser, List<int> idTicketFiles)
        {
            ...
            try
            {
                ...
                if (ticket != null && userLogin != null)
                {
                    ...
                    history.HasBalance = false;
                    history.IdSpecialAgentBalance = null;
        ...
        public async Task<Response<bool>> TicketToDispatch(int idTicketHistory,int idTicket,string quality,string qualityTranslator,string qualityTypist, List<UserCode> otherUsers)
        {
            ...
            try
            {
                ...
                if(ticket != null)
                {
                    ...
                    if(ticket.TicketHistories.Any(x=>x.AsignationType=="RF" && x.Flag == false))
                    {
                        ticket.TicketHistories.Add(new TicketHistory
                        {                           
                            UserFrom = "42",
                            ...
                            Flag = false,
                            Cycle = code,
                            IdStatusTicket = (int)TicketStatusEnum.Por_Despachar_F_R
                        });
                        ticket.IdStatusTicket = (int)TicketStatusEnum.Por_Despachar_F_R;
                    }
        ...
        public async Task<Response<bool>?> ConfirmAgentHistory(int idTicketHistory,int idSpecialPrice)
        {
            var response = new Response<bool>();
            using var context = new SqlCoreContext();
            try
            {
                ...
                var ticket = await context.Tickets.Where(x => x.Id == ticketHistory.IdTicket).FirstOrDefaultAsync();

                if (ticketHistory != null)
                {
                    if (idSpecialPrice > 0)
                    {
                        ticketHistory.HasBalance = true;
                        ticketHistory.IdSpecialAgentBalance = idSpecialPrice;
                    }
